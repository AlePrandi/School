...
int convertiData(Data attuale)
{
    int dataCorrente = attuale.anno * 1000 + attuale.mese * 100 + attuale.giorno;
    return dataCorrente;
}
...
int cercaScaduti(Prodotto archivio[], int nProdotti, Data data, FILE *fp)
{
    ...
    for (int k = 0; k < nProdotti; k++)
    {
        if (convertiData(data) > convertiData(archivio->data_scadenza))
        {
...
int main()
{
    ...
    do
    {
        printf("Inserisci la data attuale(anno mese giorno): ");
        scanf("%d %d %d", &data.anno, &data.mese, &data.giorno);
    } while (data.anno > ANNO_MAX || data.mese > MESE_MAX || data.giorno > GIORNO_MAX);
    ...
    {
        printf("Impossibile aprire il file 'scaduti.dat'.\n");
    }
