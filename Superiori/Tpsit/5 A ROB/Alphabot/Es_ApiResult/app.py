...
def auth_required_resource(func):
    """Decoratore per classi Resource di Flask-RESTful per richiedere autenticazione"""
...
def auth_required(view_function):
    """Decoratore per route tradizionali Flask per richiedere autenticazione"""
...
class LineSensorResource(Resource):
    @auth_required_resource
    def get(self):
...
class MovementResource(Resource):
    ...
    def post(self):
        ...
        parser.add_argument('movement_id', type=str, required=True, 
                           help='ID del movimento è obbligatorio')
        parser.add_argument('state', type=bool, default=True,
                           help='Stato del movimento (True/False)')
        ...
        if movement_id not in MOVEMENT_COMMANDS:
            return {
                "success": False, 
                "error": f"ID movimento non valido: {movement_id}"
            }, 400
...
class SpeedResource(Resource):
    ...
    def post(self):
        ...
        parser.add_argument('speed', type=int, required=True,
                          help='Velocità del motore è obbligatoria')
        args = parser.parse_args()
        ...
        if 20 <= speed <= 100:
            ...
            return {
                "success": False, 
                "error": "Valore velocità non valido"
            }, 400

class KeyEventResource(Resource):
    @auth_required_resource
    def post(self):
        """
        ...
        parser.add_argument('key', type=str, required=True, 
                           help='Il tasto è obbligatorio')
        parser.add_argument('state', type=bool, required=True,
                           help='Lo stato del tasto (True/False) è obbligatorio')
        args = parser.parse_args()
...
class TokenInfoResource(Resource):
    def get(self):
        """Mostra le informazioni del token"""
        token = request.cookies.get('token')
        if not token:
            return {"message": "Token non trovato!"}, 400
...
# --- Registrazione delle risorse RESTful ---

# Risorse per il controllo del robot
api.add_resource(MovementResource, '/api/movement')
api.add_resource(KeyEventResource, '/api/key_event')
api.add_resource(SpeedResource, '/api/speed')

# Risorse per i sensori
api.add_resource(LineSensorResource, '/api/sensors/line')

# Informazioni sul token
api.add_resource(TokenInfoResource, '/api/token_info')

# --- Route tradizionali Flask ---
...
# --- Mantenimento delle vecchie route per compatibilità ---

@app.route("/key_event", methods=['POST'])
@auth_required
def key_event():
    """Route legacy per compatibilità con il frontend esistente"""
    data = request.json
    key = data.get('key')
...
def token_info():
    """Route legacy per compatibilità con client esistenti"""
    token = request.cookies.get('token')
    if not token:
